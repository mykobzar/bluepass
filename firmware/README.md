...
| Version | Date | Notes |
|---|---|---|
| **2.0.8-beta** | 2026-05-27 | FIDO2: fix CTAPHID_INIT response CID — send on broadcast channel per CTAP2.0 §8.1.5.4 |
| **2.0.8-beta** | 2026-05-27 | FIDO2: extend diagnostic log to CTAPHID layer (packet RX, INIT, errors) |
| **2.0.8-beta** | 2026-05-27 | FIDO2: add diagnostic log to Passkey settings tab (GET/DELETE /api/passkey/diag) |
...
|---|---|---|
| `bootloader-2.0.8-beta.bin` | `0x0` | Second-stage bootloader |
| `partition-table-2.0.8-beta.bin` | `0x8000` | Partition layout (NVS + dual OTA slots) |
| `ota_data_initial-2.0.8-beta.bin` | `0x10000` | OTA slot selector (initial state) |
| `bluepass-2.0.8-beta.bin` | `0x20000` | Main application |

### With flash encryption (recommended)

| File | Flash address | Description |
|---|---|---|
| `bootloader-2.0.8-beta-enc.bin` | `0x0` | Bootloader with encryption support |
| `partition-table-2.0.8-beta-enc.bin` | `0x8000` | Partition layout |
| `ota_data_initial-2.0.8-beta-enc.bin` | `0x10000` | OTA slot selector |
| `bluepass-2.0.8-beta-enc.bin` | `0x20000` | Main application (encryption-enabled build) |
